...
import {
    ...
    Copy, File, FileCode2, Folder, FolderOpen, GitCompare, Library,
...
type ModuleFileKind = 'folder' | 'server_script' | 'client_script' | 'shared_module' | 'ui';
...
type Conflict = { kind: 'project'; title: string; fileId: string; browserSource: string; serverSource: string; serverRevision: number };
...
function StudioTreeNode({ node, onExpand, onRename }: { node: StudioNode; onExpand: (node: StudioNode) => void; onRename: (node: StudioNode) => void }) {
    ...
    return <div className="select-none text-xs">
        <button type="button" onDoubleClick={() => onRename(node)} onClick={toggle} title="Double-click to rename" className="flex h-7 w-full items-center gap-1.5 rounded px-1.5 text-left text-slate-300 outline-none hover:bg-white/[0.05] hover:text-white focus-visible:ring-1 focus-visible:ring-sky-400">
            {node.hasChildren ? (open ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />) : <span className="w-3" />}
            <Boxes className="h-3.5 w-3.5 text-fuchsia-300" />
            <span className="truncate">{node.name}</span>
        </button>
        {open && node.children && <div className="ml-3 border-l border-white/8 pl-1">{node.children.map((child) => <StudioTreeNode key={child.id} node={child} onExpand={onExpand} onRename={onRename} />)}</div>}
    </div>;
}
...
function UiTree({ value, depth = 0 }: { value: unknown; depth?: number }) {
    ...
    return <div className={depth ? 'ml-4 border-l border-white/8 pl-2' : ''}>
        <div className="flex h-7 items-center gap-2 text-xs text-slate-300"><Boxes className="h-3.5 w-3.5 text-fuchsia-300" /><span>{node.name || 'UI object'}</span></div>
...
export default function ModuleIdeClient() {
    ...
    useEffect(() => {
        ...
        setTabs([]); setActiveTab(''); setDiagnostics({}); setServerProblems([]);
    ...
    const hasDirty = Object.values(drafts).some((draft) => draft.dirty);
    ...
    }, [log, moduleId, setDrafts, setProject]);
    ...
    const processEvents = useCallback(async (events: BridgeEvent[]) => {
        for (const event of events) {
            ...
            else if (event.type === 'ui.renamed') { setStudioRoots((roots) => mergeStudioNode(roots, { ...event, type: 'tree.instanceRenamed' })); log(`Renamed UI to ${String(event.payload.name || 'UI')}.`, 'studio', 'success'); }
    ...
    const renameStudioUi = useCallback((node: StudioNode) => {
        const name = window.prompt('UI name', node.name)?.trim();
        if (!name || name === node.name) return;
        void sendEvents([{ type: 'ui.rename', requestId: crypto.randomUUID(), payload: { instanceId: node.id, name } }]);
    }, [sendEvents]);
    ...
    const createItem = useCallback(async () => {
        ...
        if (!name) return;
        name = name.replace(/\.luau$/i, '');
        if (newItem.kind === 'server_script' && !name.endsWith('.server')) name += '.server';
        if (newItem.kind === 'client_script' && !name.endsWith('.client')) name += '.client';
        const path = baseParent ? `${baseParent}/${name}` : name;
        const sourceCode = newItem.kind === 'folder' ? null : newItem.kind === 'shared_module' ? 'module {\n\n}\n' : '';
    ...
    useEffect(() => {
        const onKey = (event: KeyboardEvent) => {
            ...
            if (event.key.toLowerCase() === 's' && activeFile) { event.preventDefault(); void saveProjectFile(activeFile.id); }
        };
        window.addEventListener('keydown', onKey); return () => window.removeEventListener('keydown', onKey);
    }, [activeFile, saveProjectFile]);
    ...
    return <div className="relative flex h-screen min-h-[640px] flex-col overflow-hidden bg-[#0b0f17] text-slate-100">
        ...
        <header className="flex h-14 shrink-0 items-center gap-3 border-b border-white/8 bg-[#111722] px-3">
            ...
            <div
                ...
                className="w-44 min-w-0 max-w-64 truncate rounded-lg border border-white/10 bg-[#0b0f17] px-3 py-2 text-sm font-semibold text-white max-[700px]:w-32"
            >
        ...
        <main className="grid min-h-0 flex-1" style={{ gridTemplateColumns: `${compactLayout ? Math.min(leftWidth, 230) : leftWidth}px 4px minmax(0,1fr) ${showRightPanel ? `4px ${rightWidth}px` : '0 0'}` }}>
            <aside className="min-h-0 overflow-y-auto bg-[#0e141e]">
                ...
                <div className="border-t border-white/8 p-2"><div className="mb-2 flex items-center justify-between px-1"><span className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500">Studio UI</span><button disabled={!connected} onClick={() => void sendEvents([{ type: 'tree.snapshot', payload: {} }])} className="rounded p-1 text-slate-500 hover:text-white disabled:opacity-30"><RefreshCw className="h-3.5 w-3.5" /></button></div>{studioRoots.length ? studioRoots.map((root) => <StudioTreeNode key={root.id} node={root} onExpand={expandStudioNode} onRename={renameStudioUi} />) : <p className="rounded border border-dashed border-white/10 p-3 text-xs leading-5 text-slate-500">Connect Roblox Studio to browse StarterGui names. Double-click a name to rename it.</p>}</div>
            </aside>
            ...
            <section className="grid min-h-0" style={{ gridTemplateRows: `minmax(0,1fr) 4px ${bottomHeight}px` }}>
                <div className="flex min-h-0 flex-col bg-[#090d14]">
                    <div className="flex h-10 shrink-0 overflow-x-auto border-b border-white/8 bg-[#111722]">
                        {tabs.map((key) => { const file = project?.files.find((item) => item.id === key.slice(8)); const dirty = file ? drafts[file.id]?.dirty : false; return <button key={key} onClick={() => setActiveTab(key)} className={`group flex min-w-28 max-w-52 items-center gap-2 border-r border-white/8 px-3 text-xs ${activeTab === key ? 'border-t-2 border-t-sky-400 bg-[#090d14] text-white' : 'text-slate-500 hover:text-slate-200'}`}>{file ? fileIcon(file) : <File className="h-3 w-3" />}<span className="truncate">{file?.name || 'Deleted file'}</span>{dirty ? <span className="ml-auto h-2 w-2 rounded-full bg-amber-300" /> : <X onClick={(event) => { event.stopPropagation(); closeTab(key); }} className="ml-auto h-3 w-3 opacity-0 group-hover:opacity-100" />}</button>; })}
                    ...
                    {activeFile && activeDraft ? <>
                        ...
                        <div className="min-h-0 flex-1"><ModuleIdeEditor value={activeDraft.value} path={activeFile.path} language="luau" projectPaths={project?.files.map((file) => file.path) || []} onChange={(value) => setDrafts((current) => ({ ...current, [activeFile.id]: { ...current[activeFile.id], value, dirty: value !== activeFile.sourceCode, status: value !== activeFile.sourceCode ? 'dirty' : 'saved' } }))} onSave={() => void saveProjectFile(activeFile.id)} onDiagnostics={(items) => setDiagnostics((current) => ({ ...current, [activeFile.path]: items }))} /></div>
                    </> : activeFile?.kind === 'ui' ? <div className="min-h-0 flex-1 overflow-auto p-4"><p className="mb-3 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">UI names</p><UiTree value={activeFile.uiTree} /><p className="mt-4 text-xs text-slate-500">UI properties stay in Roblox Studio and are not exposed or editable here. Rename the imported UI file from the Explorer.</p></div>
