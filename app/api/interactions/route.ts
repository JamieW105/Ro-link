...
type RobloxUserProfile = RobloxSearchCandidate & {
    description?: string | null;
    created?: string | null;
    isBanned?: boolean | null;
};

type ResolvedRobloxUser = {
    id: number | string;
    name: string;
    displayName: string;
...
function mergeRobloxHeaders(headers?: HeadersInit) {
    const merged = new Headers(ROBLOX_API_HEADERS);
...
async function fetchRobloxJsonFromOrigins<T>(
    origins: string[],
    path: string,
    init: RequestInit,
    label: string,
): Promise<T> {
    const failures: Array<{ origin: string; status?: number; message?: string }> = [];
...
async function resolveRobloxUser(input: string): Promise<ResolvedRobloxUser> {
    ...
    if (/^\d+$/.test(searchInput)) {
        const profile = await fetchRobloxJsonFromOrigins<RobloxUserProfile>(
    ...
    const usernameData = await fetchRobloxJsonFromOrigins<{ data?: RobloxSearchCandidate[] }>(
        ROBLOX_USER_API_ORIGINS,
        '/v1/usernames/users',
        {
            method: 'POST',
    ...
    const searchData = await fetchRobloxJsonFromOrigins<{ data?: RobloxSearchCandidate[] }>(
        ROBLOX_USER_API_ORIGINS,
...
async function findVerifiedRobloxIdentity(input: string): Promise<VerifiedRobloxIdentity | null> {
    const searchInput = String(input ?? '').trim();
    if (!searchInput) {
        return null;
    }
    ...
    const { data, error } = await query.maybeSingle<VerifiedRobloxIdentity>();
    if (error) {
...
function buildVerifiedRobloxFallback(identity: VerifiedRobloxIdentity, input: string): ResolvedRobloxUser | null {
...
async function findLiveRobloxIdentity(serverId: string, input: string): Promise<ResolvedRobloxUser | null> {
    const liveServers = await fetchLiveServers(serverId);
...
async function fetchRobloxLookup(username: string, serverId: string, openCloudKey?: string | null): Promise<RobloxLookupResult> {
    ...
    let matchedUser: ResolvedRobloxUser;
    try {
        ...
        console.warn('[LOOKUP] Roblox public lookup failed; using local fallback:', error instanceof Error ? error.message : error);
        matchedUser = fallbackUser;
    }

    const apiKey = String(openCloudKey ?? '').trim();
    const [legacyProfile, cloudProfile, thumbnailData] = await Promise.all([
        matchedUser.profile
            ? Promise.resolve(matchedUser.profile)
            : fetchRobloxJsonFromOrigins<RobloxUserProfile>(
                ROBLOX_USER_API_ORIGINS,
        ...
        fetchRobloxJsonFromOrigins<{ data?: Array<{ imageUrl?: string }> }>(
