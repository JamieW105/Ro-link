...
export async function GET() {
    ...
    const { data, error } = await client
        .from('addon_modules')
        .select('id, slug, name, description, thumbnail_url, thumbnail_urls, version, category, status, created_at, updated_at, published_at')
...
export async function POST(req: Request) {
    ...
    }).select('id, slug, name, description, thumbnail_url, thumbnail_urls, version, category, status, created_at, updated_at').single();
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
